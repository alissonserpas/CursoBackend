...
const editarNota = (title, body) => {
    const notas = cargarNotas();
    const notaIndex = notas.findIndex((note) => note.title === title);

    if (notaIndex !== -1) {
        notas[notaIndex].body = body;
        guardarNotas(notas);
        console.log(chalk.green.inverse('Nota editada correctamente'));
    } else {
        console.log(chalk.red.inverse('No se encontró una nota con ese título'));
    }
};
